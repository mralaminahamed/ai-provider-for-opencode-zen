# Contributing to AI Provider for OpenCode Zen
...
Search [existing issues](https://github.com/mralaminahamed/ai-provider-for-opencode-zen/issues) before filing a new
...
```bash
composer analyze       # phpcs + phpstan (level max)
composer test          # PHPUnit
```

An API key is a credential. It is stored server-side and must never reach the
browser, a log, an error message, or a REST response — if a change could put it
in any of those, it needs a test saying it does not.
